...
import { useMemo, useState, memo, useEffect, useDeferredValue, startTransition } from 'react';
...
const StatsPage = ({ items, totalIncome, totalExpenses, remaining }: StatsPageProps) => {
  ...
  // Utiliser useDeferredValue pour ne pas bloquer l'UI pendant les calculs
  const deferredItems = useDeferredValue(items);

  useEffect(() => {
    // Différer le rendu avec startTransition
    startTransition(() => {
      setIsClient(true);
    });
  }, []);
  ...
  const predictions = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const monthExpenses = deferredItems
